...
### Step 0 — 自动更新数据（每次预测前强制执行）

**预测之前必须先同步最新数据。** 后台静默执行，无需用户触发：

1. 同步已完赛比分，更新预测命中记录
2. 刷新竞彩赔率（如缓存超过 6 小时）
3. 刷新积分榜和赛程（如缓存超过 5 分钟）

不等待用户说"同步"——每次预测自动触发。更新完成后简要告知用户"数据已更新至 XX:XX"。
...
| "同步" | 赛后比分同步 |
| "夺冠概率" | 蒙特卡洛 5000 次模拟 |
| "怎么买"/"投注方案" | 生成 100 元预算最优分配 |
| "串关"/"来个串子" | 2串1稳胆 + 3串1博高赔 |
| "能回本吗"/"风险多大" | 展示盈亏情景分析 |
